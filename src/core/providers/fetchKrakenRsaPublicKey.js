...
export async function fetchKrakenRsaPublicKey(platform, bhvrSessionCookie, apiKey) {
    ...
    if (!Array.isArray(result)) {
        throw new Error('Unexpected configuration response format.');
    }

    const rsaPublicKeyEntry = result.find(entry => entry.keyName === 'KRAKEN_RSA_PUBLIC_KEY');

    if (!rsaPublicKeyEntry || typeof rsaPublicKeyEntry.value !== 'string') {
        throw new Error('"KRAKEN_RSA_PUBLIC_KEY" not found in configuration response.');
    }

    return rsaPublicKeyEntry.value;
}